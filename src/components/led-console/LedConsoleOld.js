...
const itemFactory = (num) => {
  const items = config.leds.map((led, index) => {
    const ref = createRef();
...
const initItems = itemFactory(149);

const LedConsole = () => {
  //   const { composeSelectable } = useDragSelect();
  ...
  return (
    <Paper>
      ...
      >
        ...
        <div className="shelf">
          {items.map(({ Component, ref, id, color }) => {
        ...
        <h1 style={{ color: "white" }}>{selectedItems.length}</h1>
      </div>
    </Paper>
  );
};
